...
function RunTab({
  ...
  const parseDebugSteps = (value: unknown): CanvasRunDebugStep[] => {
    ...
    return value
      ...
      .map((raw) => ({
        ...
        screenshotDataUrl:
          typeof raw.screenshotDataUrl === "string" ? raw.screenshotDataUrl : undefined,
      }));
  };
  ...
  const appendDebugSteps = (steps: CanvasRunDebugStep[]) => {
    ...
    const debugLogs: RunLogEntry[] = steps.map((step): RunLogEntry => ({
      ...
      msg: `${step.detail || step.label || step.action}${step.screenshotDataUrl ? " · screenshot captured" : ""}`,
  ...
  const run = async () => {
    ...
    if (macro.sourceType === "canvas") {
      ...
      try {
        ...
        const courses: Course[] = Array.isArray(payload.courses) ? payload.courses : [];
        const assignments: Assignment[] = Array.isArray(payload.assignments) ? payload.assignments : [];
        ...
        {macros.map((m) => (
          ...
          >
            ...
            <div className={`flex items-center gap-2 mt-0.5 text-xs ${
              selectedId === m.id ? "text-white/60" : "text-[var(--text-muted)]"
            }`}>
...
export default function MacroPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-[calc(100vh-3.5rem)]">
        <div className="flex flex-col h-full min-h-0 overflow-hidden">
          {/* Mobile tab bar */}
          <div className="lg:hidden flex border-b border-[var(--border)] bg-[var(--bg)]">
          ...
          <div className="flex-1 min-h-0 flex overflow-hidden">
            {/* Sidebar */}
            <aside
              className={`shrink-0 bg-[var(--bg)] lg:w-72 lg:border-r lg:border-[var(--border)] flex-col ${
                mobileTab === "sidebar" ? "flex w-full" : "hidden lg:flex"
              }`}
            >
              <Sidebar
                macros={macros}
